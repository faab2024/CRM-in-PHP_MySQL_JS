...
if(isset($_POST["import"])){
 ...
 //check selected file extension is present in allowed extension array
  ...
  $output .= "<label class='text-success'><center><h2>Leads Are Registered Successfully</h2></center></label><br /><table class='table table-bordered'>";
  ...
  foreach ($objPHPExcel->getWorksheetIterator() as $worksheet){
   ...
   for($row=2; $row<=$highestRow; $row++){
    ...
    $query="INSERT INTO leads(name,email,phone,location,client,business_phone,website,business_name,lead_status,business_industry,lead_score) 
